...
<body>
		...
		<div class="container">
			...
			<div class="login">
				<a class="nav-link text-light mx-lg-2 mx-4"
					href="/Cinema_Megarama_war_exploded/login">Login</a>
			</div>
			...
			<c:forEach var="film" items="${filmlist}">
				<div class="image">
					<img style="width: 100%; height: 100vh;"
						src="${film.getMegaCover()}" alt="">
					<div class="example">
						<div class="description" style="display: flex; margin: 10px;">
							<img src="${film.getCover()}" class="backimg" />
							<div
								style="margin: 15px; display: flex; flex-direction: column; justify-content: space-between">
								<div style="width: 350px">
									<h3 style="color: white;">${film.getTitle()}</h3>
									<p style="font-size: 13px; text-align: start;">${film.getDuration()}
										-<span> ${film.getReleaseDate()} -</span><span
											style="font-weight: bold;">${film.getDirector()}</span>
									</p>
									<p style="font-size: 13px; text-align: start;">
										<span class="text-success">92% Match </span><span
											class="border rounded px-1 mx-2">TV - MA</span><span
											style="font-weight: bold;" class="border rounded px-1 mx-1 ">HD</span><span
											style='color: #fdb000; background: rgba(0, 0, 0, 0.6);'
											class="rounded border px-2 mx-1">9.0 <i
											class='bx bxs-star' style='color: #fdb000'></i></span>
									</p>
									<p style="font-size: 15px; text-align: start;">Genre :
										${film.getGenre()}</p>
								...
								<div class="btns"
									...
									<button class="bte">
										<a style="color: white; text-decoration: none;" type="submit"
											href="BookMovie?id=${film.getFilmId()}">Book Now</a>
									</button>
	...
	<h2
		style="color: white; margin: 1rem; padding: 10px 20px; background: background: linear-gradient(0deg, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.6) 20%, rgba(0, 0, 0, 1) 50%, rgba(0, 0, 0, 0.6) 80%, rgba(0, 0, 0, 0) 100%);">Streaming
		Now :</h2>
			...
			<div class="card-container box">
				<div class="card-flip">
					...
					<div class="card front">
						<img src="${film.getCover()}" class="card-img-top img-fluid"
							alt="movie-poster">
						<div class="card-block">
							<h5 class="card-title">${film.getTitle()}</h5>
							<p class="card-text">${film.getGenre()}</p>
							<p class="card-text">
								<small class="text-muted">${film.getDuration()} min </small>
							</p>
						</div>
					</div>
					<!-- End Card Front -->
